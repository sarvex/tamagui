...
Here is a demo with more advanced animations using [AnimatePresence](/docs/core/animations#animatepresence-and-exitstyle) and [`<Tabs.Tab />`](#tab)'s `onInteraction` prop.
...
Container for the tabs. Supports scrolling by extending [Group](/docs/components/group). You can disable passing border radius to children by passing `disablePassBorderRadius`.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: `Used for making custom indicators when the tab's interacted with`,
    },
    {
      name: 'unstyled',
      type: `boolean`,
